...
- Конфиденциальные `.docx` обрабатываются по сценарию «маска → анализ в LLM → сборка → восстановление». Конфиденциальные данные в LLM не уходят.
...
- Управляемая стоимость: Codex по подписке, API — только когда нужно. Возможно подключение других нейросетей.
